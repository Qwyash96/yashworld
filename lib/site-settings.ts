...
export async function getSiteBranding(): Promise<SiteBranding> {
  const snapshot = await getAdminDb().collection(COLLECTION).doc(BRANDING_DOC_ID).get()
  if (!snapshot.exists) return {}
  return snapshot.data() as SiteBranding
}

export async function saveSiteBranding(patch: Partial<SiteBranding>): Promise<SiteBranding> {
  const ref = getAdminDb().collection(COLLECTION).doc(BRANDING_DOC_ID)
  await ref.set({ ...patch, updatedAt: new Date().toISOString() }, { merge: true })
  const snapshot = await ref.get()
  return snapshot.data() as SiteBranding
}
