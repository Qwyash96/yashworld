...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="group flex h-full flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg sm:rounded-3xl">
      <Link href={`/products/${product.id}`} className="relative block aspect-[4/5] shrink-0 overflow-hidden bg-[#f5f7f3]">
        ...
        <button
          ...
          className="absolute right-2 top-2 flex size-8 items-center justify-center rounded-full bg-white/95 shadow-sm transition hover:scale-110 sm:right-2.5 sm:top-2.5 sm:size-9"
        >
          <Heart className={cn("size-4 sm:size-5", wishlisted ? "fill-red-500 text-red-500" : "text-gray-500")} />
        </button>

        <div className="absolute bottom-2 right-2 flex gap-1 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
          <button
            onClick={(e) => {
              e.preventDefault()
            ...
            className="flex size-7 items-center justify-center rounded-full bg-white/95 shadow-sm"
          >
